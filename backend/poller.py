"""
Polling-based Facebook auto-reply bot for AR TechLabs.
...
from ai import generate_comment_reply, generate_inbox_reply, is_list_request, SERVICE_LIST
...
# One post per evening — pulled from the Google Sheet queue (30-day content
# calendar, see posts.txt). Fires once within a 10-minute window after 19:00.
AUTO_POSTS = ["19:00"]
...
def check_inbox(reply=True):
    ...
    # Pricing/package request → raw service list (no AI, no hallucination)
    if is_list_request(user_text):
        send_message(sender_id, SERVICE_LIST)
        return
...
def main():
    logger.info("AR TechLabs — FB Bot starting...")
    ...
    logger.info("Auto-post schedule: %s BD time (evening), from Google Sheet queue.",
                ", ".join(AUTO_POSTS))
